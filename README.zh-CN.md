...
每次修改账号池前，插件都会重新读取目标 provider，然后再执行 PATCH。同一浏览器的多个标签页通过 Web Lock 串行执行修改。启用 `auto_pool: true` 后，请求时 scheduler 会跳过已识别的配额耗尽凭据，不会重写 provider 配置。reset 时间到达后，凭据会自动恢复资格；**Manual hold** 始终优先。
...
- 收到可识别的 5 小时或 Weekly 配额耗尽错误后，对应凭据会在内存中暂存到 reset 时间；时间到达后自动重新可用。
...
插件不负责浏览器账号登录、数据库、New API 接入或自动停用凭据。自动暂存不会重写 provider 配置。账号注册表会有意以明文保存一次性账号凭据，并使用 `0600` 文件权限保护。因此它只适合可信管理员控制的自托管环境。
