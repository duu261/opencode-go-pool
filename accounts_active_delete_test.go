...
func TestAccountRouteRejectsDeletingEnabledPoolAccount(t *testing.T) {
	dir := t.TempDir()
	...
	if err := accounts.Save(accountsPath, []accounts.Account{{APIKey: "active-secret", ProviderName: "opencode-direct", Label: "active"}}); err != nil {
	...
	body, _ := json.Marshal(map[string]any{"revision": revision, "accounts": []accounts.Account{}})
	...
	if len(stored) != 1 || stored[0].APIKey != "active-secret" {
		t.Fatalf("active account was deleted: %#v", stored)
	}
}
